...
## DEC-058 — Carga perezosa de harnesses por handoff (modelo "switch") + comando conductor genérico `/faro-run` (sesión 42)
**Fecha:** 2026-06-13
**Decisión:** El despliegue de agentes/skills/código de cada harness en un proyecto cliente pasa de "instalar todo de una vez" a **carga perezosa just-in-time por fase**, con estos componentes:
1. **`faro-setup.ps1` instala solo el harness de arranque (010), no los 11.** Hoy `faro-setup` copia **todos** los `.claude/agents/*.md` y skills del repo (descubierto en sesión 42), lo que infla el contexto del sistema: Claude Code carga la **descripción** de cada agente y skill al iniciar la sesión, aunque la fase activa no los use. `faro-setup` conserva el andamiaje del proyecto (CLAUDE.md, settings, commands, workflows, `800_inputs/brief.md`) y **delega la parte de agentes/skills/scripts a `deploy-harness.ps1 -Harness 010`**. Modelo uniforme: todo harness llega por el mismo camino.
2. **Modelo "switch" (agregar + podar el previo).** Al pasar a un harness se instala el nuevo y se **remueven** los agentes/skills del anterior. Es seguro porque la cadena del pipeline es **lineal y se comunica por disco**: cada harness deja su handoff persistido (eventos, `client_config`, Bronce) y el siguiente lo lee del filesystem — **no necesita los agentes vivos del previo**. Si hay que re-correr un harness anterior, se re-despliega. **Matiz:** los harnesses transversales/permanentes (045 Monitor, 050 Lifecycle, 055 Command) podrían requerir un **conjunto base permanente** que coexista con el activo; el switch puro aplica a la cadena lineal de construcción/operación (010→015→020‖025→030→035→040). Se afinará al construir esos harnesses.
3. **`deploy-harness.ps1` gana tres capacidades:** (a) **copia recursiva de `scripts/{NNN}_{prefijo}/pipeline/`** — hoy `deploy-harness.ps1:158` solo copia archivos de primer nivel, dejando fuera el paquete `pipeline/` que el Worker necesita (hueco bloqueante del PASO 16 del 015); (b) **poda del harness previo** (switch); (c) **mapa "harness actual → siguiente"** (orden DEC-026 + cadena del pipeline; el único punto de bifurcación es el fan-out 020 ‖ 025, donde se ofrecen ambos).
4. **Comando conductor genérico `/faro-run`.** Reemplaza la proliferación de comandos por-harness (`/faro-discovery`, `/faro-intake`, …). Detecta el **único** `*-governor.md` instalado (con el modelo switch solo hay uno → detección inequívoca) y lo conduce, sirviendo tanto para **arrancar** (`MODO: INIT`) como para **continuar** (el governor lee el estado y decide Inicio vs Continuación). El usuario teclea siempre lo mismo tras cada reinicio.
5. **Transición disparada por el conductor con una confirmación.** Coherente con DEC-051: al cerrar un harness (`PHASE_COMPLETE`), el governor señala "siguiente disponible"; el conductor pregunta *"¿despliego el {siguiente}?"*, y con el **"sí"** del operador ejecuta `& "$env:FARO_HOME\deploy-harness.ps1" -Harness {siguiente} -Destino .` (+ poda). **`faro-setup` ya escribe `FARO_HOME` en `settings.local.json`**, así que el proyecto sabe dónde está el repo. Después, **el operador debe reiniciar Claude Code**: los agentes/skills se escanean al **iniciar** la sesión (no en caliente), así que el reinicio es lo que activa el nuevo harness y resetea el contexto al mínimo. UX por salto: *"sí" → reiniciar → `/faro-run`*.
**Razón:** El operador pidió explícitamente no inflar el contexto con harnesses que no se usan en la fase activa, y que la transición sea fluida (una confirmación + reinicio). El modelo de archivos de Claude Code (escaneo de agentes/skills al inicio de sesión) hace del reinicio el momento natural de reset de contexto, no un parche. La sesión 42 confirmó además que `faro-setup` sobre-instalaba y que `deploy-harness` no copiaba `pipeline/`.
**Impacto:** Archivos a modificar (tareas T-185..T-190): `deploy-harness.ps1` (recursivo + poda + mapa), `faro-setup.ps1` (delegar al 010), nuevo `commands/faro-run.md` (genérico), ajuste del cierre de `discovery-governor.md` (ofrecer el salto + instrucción de reinicio), y deprecación de `/faro-discovery` (lo absorbe `/faro-run`). Prerrequisito del **PASO 16** del 015 (la corrida e2e necesita el `pipeline/` desplegado). No afecta el código del pipeline ni la rúbrica.

---

## DEC-059 — Corrida e2e 010→015 en `Test_007_Conservas`; esta terminal es la de supervisión; convención `800_inputs/` para datos del 015 (sesión 43)
**Fecha:** 2026-06-13
**Decisión:** El PASO 16 del 015 (smoke test e2e) se ejecuta como una corrida de **cadena completa 010→015** sobre un cliente ficticio único, con esta organización de dos terminales (LEC-053):
1. **Terminal de ejecución** = un proyecto cliente dedicado llamado **`Test_007_Conservas`** (en `Test_Forecaster/`), donde corren de verdad los harnesses 010 y luego 015 (deploy, agentes, gates, evaluador). Es una **sesión de Claude Code distinta**.
2. **Terminal de supervisión/apoyo** = **esta** sesión, sobre el repo fuente `Harness_Forecaster`. Su papel durante la corrida es **observar el comportamiento del harness del 010 al 015, responder dudas del operador, auditar artefactos (Bronce, manifest, eventos, verdict), diagnosticar y aplicar fixes al código fuente** si algo falla. **No ejecuta el harness aquí.** Aplica el patrón de las corridas del 010 (el repo fuente fue siempre el soporte de Test_004A/005/006).
3. **Cliente ficticio:** **Conservas del Pacífico S.A.S.** (Cali, Colombia) — conservas de alimentos B2B, escala mediana (M/L), ~350 SKUs, ~45 clientes, ~3 años de historial **declarado** (rango real ~2,2 años → dispara el warning de rango a propósito). Entrega **solo Pedidos** (`tiene_esquema2 = false`).
4. **Convención `800_inputs/` (Fase 1):** el operador deja **todos** los archivos de entrada del cliente en `800_inputs/` — `brief.md` (lo lee el 010) y `orders.csv` (lo lee el 015). Precisa el punto suelto de `brief/015_intake.md:56` ("el operador deja el archivo en la carpeta del tenant") nombrando la subcarpeta. El governor arma el Sprint Contract con `snapshot_esquema1: 800_inputs/orders.csv`. **El Bronce NO se pega a mano** — lo escribe el 015 (copia bit-exacta en `…/tenants/{id}/1000_data/005_bronze/`).
5. **Material de prueba (en el repo fuente, `test_data/015_intake_e2e/`):** `brief.md` (Conservas), `orders.csv` (entrega principal: ~4.050 filas, `;` cp1252 con acentos, 40 cantidades negativas + 50 duplicados internos sembrados), `orders_sin_cantidad.csv` (mini-corrida de rechazo — sin la columna de cantidad → `REJECTED_STRUCTURE`), `_build_dataset.py` (generador reproducible, semilla fija) y `README.md` (receta del flujo + convención + resultados esperados). **Validado** contra los módulos reales del pipeline (no opinión): cp1252/`;` OK, estructura aprobada vs rechazada, 40 negativas, 50 duplicados, warning de rango 26,8% (2,20 vs 3 años).
**Razón:** El operador definió explícitamente esta separación de terminales y el nombre `Test_007_Conservas`. La cadena completa valida la **costura real 010→015** (el handoff `client_config` + evento `onboarding_discovery_complete`) y el nuevo deploy "switch" (DEC-058), no solo el 015 aislado. Derivar brief y dataset del **mismo** set de parámetros garantiza consistencia por construcción.
**Impacto:** No modifica código del pipeline ni agentes. Crea `test_data/015_intake_e2e/` en el repo fuente. **Recordatorio sobre nombres de archivo:** el nombre que entrega el cliente es irrelevante para el 015 (el tipo se detecta por *magic bytes*, el esquema por el *slot* `snapshot_esquema1/2`, la validez por las *columnas*); lo único fijo es el nombre del **Bronce** (`orders_*`/`inventory_*`, según `_PREFIJO` del `bronze_writer`). Prerrequisito ya satisfecho: **T-185** (deploy copia `pipeline/`) — sin él el `intake-processor` no encontraría el código en `Test_007_Conservas`.
